...
impl HQMGameWorld {
    pub(crate) fn create_player_object(
        ...
        if let Some(i) = object_slot {
            self.objects[i] = HQMGameObject::Player(HQMSkater::new(
                i,
                team,
                start,
                rot,
    ...
    pub(crate) fn create_puck_object(
        &mut self,
        start: Point3<f32>,
        rot: Matrix3<f32>,
...
pub(crate) struct HQMGame {
    ...
    pub(crate) shootout_randomized: bool,
    pub(crate) shootout_red_score: Vec<String>,
    pub(crate) shootout_blue_score: Vec<String>,
...
impl HQMRink {
    ...
    pub fn new_red_shootout(width: f32, length: f32, corner_radius: f32) -> Self {
        ...
        let planes = vec![
            (zero.clone(), Vector3::y()),
            (Point3::new(0.0, 0.0, 35.0), -Vector3::z()),
            (zero.clone(), Vector3::z()),
            (Point3::new(width, 0.0, 0.0), -Vector3::x()),
            (zero.clone(), Vector3::x()),
        ];
    ...
    pub fn new_blue_shootout(width: f32, length: f32, corner_radius: f32) -> Self {
        ...
        let planes = vec![
            (zero.clone(), Vector3::y()),
            (Point3::new(0.0, 0.0, 61.0), -Vector3::z()),
            (Point3::new(0.0, 0.0, 26.0), Vector3::z()),
            (Point3::new(width, 0.0, 0.0), -Vector3::x()),
            (zero.clone(), Vector3::x()),
        ];
        let r = corner_radius;
    ...
    pub fn get_offside_faceoff_spot(&self, pos: &Point3<f32>, team: HQMTeam) -> HQMFaceoffSpot {
        ...
        let (lines_and_net, f1, f2, f3) = match team {
            HQMTeam::Red => (
                ...
                &self.blue_neutral_faceoff_spots,
                &self.red_neutral_faceoff_spots,
                &self.red_zone_faceoff_spots,
            ),
            HQMTeam::Blue => (
                &self.blue_lines_and_net,
                &self.red_neutral_faceoff_spots,
                &self.blue_neutral_faceoff_spots,
                &self.blue_zone_faceoff_spots,
            ),
        };
    ...
    pub fn get_icing_faceoff_spot(&self, pos: &Point3<f32>, team: HQMTeam) -> HQMFaceoffSpot {
        ...
        match team {
            HQMTeam::Red => self.red_zone_faceoff_spots[left_side].clone(),
            HQMTeam::Blue => self.blue_zone_faceoff_spots[left_side].clone(),
        }
    }
}
