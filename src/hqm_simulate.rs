use crate::hqm_game::{
    HQMBody, HQMGameObject, HQMGameWorld, HQMPuck, HQMRink, HQMSkater, HQMSkaterCollisionBall,
    ...
    for i in 0..10 {
        ...
        for puck in pucks.iter_mut() {
            ...
            if let Some((overlap_pos, overlap, normal)) =
                collision_between_sphere_and_nets(&puck.body.pos, puck.radius, rink)
            {
...
fn update_stick(
    ...
    player.stick_rot = {
        ...
        rotate_matrix_around_axis(
            &mut new_stick_rotation,
            &handle_axis,
            clamp(-player.input.stick_angle, -1.0, 1.0) * FRAC_PI_4,
        );
...
fn puck_detection(
    ...
    for (team, line) in vec![
        ...
        if line.sphere_reached_line(&puck.body.pos, puck.radius)
            && !line.sphere_reached_line(&old_puck_pos, puck.radius)
        {
    ...
    for (team, line) in vec![
            ...
            && line.sphere_reached_line(old_puck_pos, puck.radius)
        {
            ...
            events.push(event);
        } else if line.sphere_past_leading_edge(&puck.body.pos, puck.radius)
            && !line.sphere_past_leading_edge(old_puck_pos, puck.radius)
        {
    ...
    if !puck.cylinder_puck_post_collision {
        for net in vec![
            ...
            for post in net.posts.iter() {
                let collision =
                    collision_between_sphere_and_post(&puck.body.pos, puck.radius, post);
                if let Some((overlap, normal)) = collision {
                    ...
                    let mut puck_force =
                        normal.scale(overlap * 0.125) - vertex_velocity.scale(0.25);
        ...
        for net in vec![
            ...
            for post in net.posts.iter() {
                let broad_check =
                    collision_between_sphere_and_post(&puck.body.pos, 2.0 * puck.radius, post)
                        .is_some();
                if broad_check {
                    for puck_vertex in puck_vertices.iter() {
                        let collision = collision_between_puck_vertex_and_post(
                            &puck.body.pos,
                            puck_vertex,
                            post,
                        );
                        if let Some((overlap, normal)) = collision {
                            ...
                            let mut puck_force = (normal.scale(overlap * 0.5) - puck_vertex_speed)
                                .scale(0.125 * 0.5);
...
fn collision_between_sphere_and_nets(
    pos: &Point3<f32>,
    radius: f32,
    rink: &HQMRink,
...
fn collision_between_puck_vertex_and_post(
    _puck_pos: &Point3<f32>,
